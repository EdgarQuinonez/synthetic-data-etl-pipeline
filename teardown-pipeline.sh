...
confirm_destructive() {
  echo ""
  log "The following GCP resources will be PERMANENTLY DELETED:"
  log "  - Cloud SQL instance: $SQL_INSTANCE_NAME (all data lost)"
  log "  - Compute instances: $INGRESS_VM, $SOURCE_VM"
  log "  - Firewall rule: $FIREWALL_RULE"
  if [ "$YES" = "1" ]; then
    return 0
  fi
  read -rp "Type 'DELETE' to continue, or press Enter to abort: " ans
  [ "$ans" = "DELETE" ]
}
...
delete_cloud_sql() {
  ...
  if [ -z "$state" ]; then
    record "Cloud SQL $SQL_INSTANCE_NAME (already deleted)" ok
    return 0
  fi
  log "deleting Cloud SQL $SQL_INSTANCE_NAME"
  gcloud sql instances delete "$SQL_INSTANCE_NAME" --quiet >/dev/null 2>&1
  for i in $(seq 1 20); do
    [ -z "$(gcloud sql instances describe "$SQL_INSTANCE_NAME" --format='value(state)' 2>/dev/null)" ] && break
    sleep 2
  done
  if [ -z "$(gcloud sql instances describe "$SQL_INSTANCE_NAME" --format='value(state)' 2>/dev/null)" ]; then
    record "Cloud SQL $SQL_INSTANCE_NAME (deleted)" ok
    return 0
  fi
  record "Cloud SQL $SQL_INSTANCE_NAME (still present)" fail
  return 1
}

delete_instances() {
  local names missing=""
  local filter="name=${INGRESS_VM} OR name=${SOURCE_VM}"
  names="$(gcloud compute instances list --filter="$filter" --format='value(name)' 2>/dev/null | sort -u | tr '\n' ' ')"
  if [ -z "$names" ]; then
    record "compute instances ($INGRESS_VM, $SOURCE_VM already deleted)" ok
    return 0
  fi
  log "deleting compute instances: $names"
  gcloud compute instances delete $INGRESS_VM $SOURCE_VM --zone "$ZONE" --quiet >/dev/null 2>&1
  for i in $(seq 1 20); do
    [ -z "$(gcloud compute instances list --filter="$filter" --format='value(name)' 2>/dev/null)" ] && break
    sleep 2
  done
  missing="$(gcloud compute instances list --filter="$filter" --format='value(name)' 2>/dev/null | sort -u | tr '\n' ' ')"
  if [ -z "$missing" ]; then
    record "compute instances ($INGRESS_VM, $SOURCE_VM deleted)" ok
    return 0
  fi
  record "compute instances (still present: $missing)" fail
  return 1
}

delete_firewall() {
  if ! gcloud compute firewall-rules describe "$FIREWALL_RULE" >/dev/null 2>&1; then
    record "firewall rule $FIREWALL_RULE (already deleted)" ok
    return 0
  fi
  log "deleting firewall rule $FIREWALL_RULE"
  gcloud compute firewall-rules delete "$FIREWALL_RULE" --quiet >/dev/null 2>&1
  if ! gcloud compute firewall-rules describe "$FIREWALL_RULE" >/dev/null 2>&1; then
    record "firewall rule $FIREWALL_RULE (deleted)" ok
    return 0
  fi
  record "firewall rule $FIREWALL_RULE (still present)" fail
  return 1
}
...
main() {
  log "=== synthetic-data-etl-pipeline teardown ==="
  stop_nifi
  stop_tunnel
  stop_sql_proxy
  stop_source_services
  if ! confirm_destructive; then
    log "aborted: no GCP resources deleted"
    log "=== summary ==="
    if [ "${#FAILED[@]}" -eq 0 ]; then
